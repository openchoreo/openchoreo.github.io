...
// Extracts the slug segment from an /ecosystem/<slug> pathname. Returns null
// for /ecosystem/ itself or a malformed segment.
function matchPathSlug(pathname: string): string | null {
  const pathMatch = pathname.match(/\/ecosystem\/([^/]+)\/?$/);
...
export default function EcosystemItem(): ReactNode {
  ...
  const slug = useMemo(() => matchPathSlug(location.pathname), [location.pathname]);

  const plugin = plugins.find((p) => itemSlug(p) === slug);
