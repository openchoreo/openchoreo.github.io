...
```bash
curl -i http://development-default.openchoreoapis.localhost:19080/greeter-service-http/greeter/greet
```
...
```bash
kubectl get svc -n thunder thunder-service
kubectl logs -n thunder -l app.kubernetes.io/name=thunder
```
