...
The diagram below illustrates the relationship between key CI resources and its interaction with the control plane and build plane:
...
```
{registry-endpoint}/{namespace-name}-{project-name}-{component-name}-image:{version}-{git-revision}
```
