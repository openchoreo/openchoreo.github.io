...
- **An external secret store** accessible from both the Workflow Plane and Data Plane (e.g., OpenBao for local dev, or Vault/AWS Secrets Manager for production)
...
:::note
This example uses OpenBao, the default secret store in a local OpenChoreo development setup. For production, see [Secret Management](../operations/secret-management.mdx).
:::
...
- [Set up Secret Management](../operations/secret-management.mdx) for production-grade secret backends and automatic rotation
- [Container Registry Configuration](../operations/container-registry-configuration.mdx) — Configure push credentials and registry providers in detail
- [Private Git Repositories](../user-guide/workflows/ci/private-repository.md) — More on Git authentication methods (SSH, basic auth)
- [Workload Generation](../user-guide/workflows/ci/workload-generation.md) — How build outputs automatically become Workload CRs
