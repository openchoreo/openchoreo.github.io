...
You've deployed a service with:

- **Plain environment variables** set directly in the `Workload`
- **Secret environment variables** pulled from an external store via `SecretReference`
- **Inline config files** mounted into the container filesystem
- **Secret files** pulled from an external store and mounted as files
- **Per-environment overrides** applied via `ReleaseBinding`
