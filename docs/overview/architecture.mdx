...
- Validating CRD instances and resolving references (e.g., Dependencies between Components)
...
- **Component**
  ...
  - Maps to workload resources like Deployment, CronJob, StatefulSet, or Proxy.
- **Endpoint**
  - A network-accessible interface exposed by a component, including routing rules, supported protocols, and visibility scopes (e.g., `external`, `internal`, `project`).
...
- **Workflow Plane**
  - A Kubernetes cluster dedicated to executing platform-defined workflows such as CI pipelines, resource provisioning, and other automated tasks.
- **Observability Plane**
...
- **Cell**
  - A Cell is the runtime reification of a single project in a specific environment in OpenChoreo. It encapsulates all components of that project-environment deployment and controls how they communicate internally and externally through well-defined ingress and egress paths.
...
- **Northbound Ingress**
  ...
  - Endpoints with `visibility: "external"` are exposed through this ingress path.
- **Southbound Egress**
...
The Workflow Plane in OpenChoreo provides dedicated infrastructure for executing platform-defined workflows, separating workflow activities from runtime environments. It ensures that tasks such as source code compilation, container image creation, resource provisioning, and test execution are performed in a secure, isolated, and scalable environment, without interfering with the application runtime.

By default, the Workflow Plane is powered by Argo Workflows, a Kubernetes-native workflow engine. However, OpenChoreo is designed to be flexible, so you can customize the Workflow Plane to use an alternative engine like Tekton, depending on your organizational needs.

While tightly integrated, the Workflow Plane is an optional component. If you already have an existing CI system, such as GitHub Actions, GitLab CI, or Jenkins, you can continue to use it for CI instead of OpenChoreo’s built-in workflows. In this setup, OpenChoreo can ingest externally built container images and proceed with deployment and observability workflows as usual.

## Observability Plane

The Observability Plane in OpenChoreo provides centralized logs, metrics, and traces collection infrastructure across all planes, enabling platform-wide monitoring, debugging, and analytics.
It collects and aggregates logs from the Control, Data, and Workflow planes using a distributed log collection pattern provided by collection agents configured via the observability modules.
These agents run on each plane, enrich logs with metadata (such as plane, namespace, project, and component), and forward them to a central observability stack configured in the Observability Plane.

Observability logs module (defaults to observability-logs-opensearch), Observability metrics module (defaults to observability-metrics-prometheus), and Observability tracing module (defaults to observability-tracing-opensearch)
serve as the core collection and aggregation platforms, supporting full-text search, structured/unstructured log storage, metrics storage, tracing storage, configurable retention, and complex queries.
On top of this, the Observer API provides a secure, unified interface for querying logs, metrics, and traces, with fine-grained filtering by namespace, project, or component, making it easy to integrate with external tools and dashboards.
...
![](/img/homepage/openchoreo-architecture-diagram.svg)

This view illustrates the full path from source code and platform configuration through build, deployment, API exposure, and runtime observability.
...
- **[Concepts](../concepts/developer-abstractions.md)**: dive deeper into the platform abstractions
- **[Quick Start Guide](../getting-started/quick-start-guide.mdx)**: try OpenChoreo with just Docker
- **[Developer Guide](../developer-guide/projects-and-components/overview.md)**: build and deploy your applications
- **[Platform Engineer Guide](../platform-engineer-guide/deployment-topology.mdx)**: set up and govern the platform
