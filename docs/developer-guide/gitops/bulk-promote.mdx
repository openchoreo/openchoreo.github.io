...
The workflow in this guide assumes you are using the **mono repository** structure described in the [GitOps Overview](../../platform-engineer-guide/gitops/overview.md#mono-repository). If you use a different repository layout, you may need to adjust the workflow configuration accordingly.
:::
...
:::note
This workflow uses the `Workflow` and `WorkflowRun` resources. It operates across multiple Components rather than a single one, so it is a generic workflow rather than a CI workflow.
:::
...
Store the token in your secret backend. For OpenBao (default local dev setup):
...
For other backends, store the token under the key `gitops-token` in your secret provider.
...
- [Workflows Overview](../../platform-engineer-guide/workflows/overview.md) — General workflow concepts
