...
The same collected logs can also be queried by raw Kubernetes coordinates (cluster, namespace, pod, container and pod labels) instead of OpenChoreo concepts.
Platform engineers use this to read the logs of OpenChoreo's own components and the infrastructure around them. See [Platform Logs](./platform-logs.mdx).
...
The API matches the `client_id` claim in the issued JWT to identify the caller, so the new client must be granted the `observer-resource-reader` role via a bootstrap authorization mapping.
...
- [Audit Logging](./audit-logging.mdx) - Collecting and querying the audit trail
