...
| Cluster gateway              | 1        | single replica by default; holds every plane's connection in memory. Multiple replicas are available as a pre-release feature — see [Cluster Gateway HA](./cluster-gateway-ha.mdx) |
