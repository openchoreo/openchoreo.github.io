...
was from webhooks, git tags or CI job names. The releasebinding controller records the
...
1. The releasebinding controller emits `DeploymentStarted`, `DeploymentSucceeded`,
...
The chart therefore refuses to render when `observer.deliveryInsights.aggregationEnabled`
is
...
The Delivery Insights store shares the alert store's database by default, so an install
that already has the alert store configured needs nothing further. Set
`observer.deliveryInsights.storeBackend` and `storeDsn` only to place it elsewhere; a
backend that differs from the alert store's requires its own DSN.
...
Reading the metrics requires the `deliveryinsights:view` action, which is granted to the
