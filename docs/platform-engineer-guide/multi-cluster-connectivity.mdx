...
Remote clusters do **not** need their own OpenBao or secret backend. The plane CRDs reference a `secretStoreRef` on the Control Plane cluster. See [Secret Management](./secret-management.mdx) for details.
:::
...
The simplest approach is to create a LoadBalancer service for the Cluster Gateway:
...
kubectl get svc cluster-gateway-lb -n openchoreo-control-plane -w
```

Save the external IP for use in the `clusterAgent.serverUrl` when installing remote planes:

```bash
export GATEWAY_IP=$(kubectl get svc cluster-gateway-lb -n openchoreo-control-plane \
  -o jsonpath='{.status.loadBalancer.ingress[0].ip}')
echo "Cluster Gateway: wss://${GATEWAY_IP}:8443/ws"
```

<details>
<summary>Alternative: route through kgateway with TLS passthrough</summary>

If you prefer to reuse the existing kgateway LoadBalancer instead of creating a new one, enable TLS passthrough in the control plane helm values:
...
This creates a TLSRoute that passes WebSocket connections through based on the SNI hostname. The hostname must match one of the `clusterGateway.tls.dnsNames` configured above.
...
After registering the Observability Plane, tell the Data Plane and Workflow Plane where to send telemetry:
...
Then install Fluent Bit on the Data Plane cluster, pointing it at the external OpenSearch:

```bash
# On the Data Plane cluster - install the logs module with fluent-bit enabled
