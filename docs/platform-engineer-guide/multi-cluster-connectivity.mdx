...
In multi-cluster deployments, telemetry collectors on the Observability Plane can't scrape pods on remote clusters directly. You need to push telemetry from each remote plane to the Observability Plane's ingestion endpoints.

### Logs (Fluent Bit to OpenSearch)

The Observability Plane's OpenSearch instance needs to be reachable from the Data Plane cluster. Expose it with a LoadBalancer service:

```bash
# On the Observability Plane cluster
kubectl apply --context $OBS_CONTEXT -f - <<'EOF'
apiVersion: v1
kind: Service
metadata:
  name: opensearch-lb
...
spec:
  type: LoadBalancer
  selector:
    app.kubernetes.io/instance: observability-logs-opensearch
...
export OPENSEARCH_IP=$(kubectl --context $OBS_CONTEXT get svc opensearch-lb \
  -n openchoreo-observability-plane -o jsonpath='{.status.loadBalancer.ingress[0].ip}')
```

Retrieve the OpenSearch credentials from the Observability Plane cluster:

```bash
export OPENSEARCH_USER=$(kubectl --context $OBS_CONTEXT get secret opensearch-admin-credentials \
  -n openchoreo-observability-plane -o jsonpath='{.data.username}' | base64 -d)
export OPENSEARCH_PASSWORD=$(kubectl --context $OBS_CONTEXT get secret opensearch-admin-credentials \
  -n openchoreo-observability-plane -o jsonpath='{.data.password}' | base64 -d)
```

Then install Fluent Bit on the Data Plane cluster, pointing it at the external OpenSearch. Create a values file for the output configuration:
...
The Observability Plane's Prometheus has remote-write receiver enabled by default. Install the metrics module on each Data Plane cluster with `remoteWrite` pointing to the Observability Plane's Prometheus:

```bash
# Get the Observability Plane Prometheus external address
export PROMETHEUS_IP=$(kubectl --context $OBS_CONTEXT get svc openchoreo-observability-prometheus \
  -n openchoreo-observability-plane -o jsonpath='{.status.loadBalancer.ingress[0].ip}')

# Install the metrics module on the Data Plane cluster
helm upgrade --install observability-metrics-prometheus \
  ...
  --create-namespace \
  --version 0.2.5 \
  --set "kube-prometheus-stack.prometheus.prometheusSpec.remoteWrite[0].url=http://${PROMETHEUS_IP}:9091/api/v1/write"
```

This installs kube-state-metrics and Prometheus on the Data Plane cluster, which scrapes local pod metrics (CPU, memory, resource requests/limits) and pushes them to the central Prometheus via remote-write. The observer can then join `kube_pod_labels` with `container_memory_working_set_bytes` for resource metrics queries.

### Configuration Reference

| Telemetry | Data/Workflow Plane Helm Value                                | Target                                                      |
| --------- | ------------------------------------------------------------- | ----------------------------------------------------------- |
