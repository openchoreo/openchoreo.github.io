...
:::warning Multi-cluster: Gateway API CRD migration (control plane and observability plane)
In a **multi-cluster** deployment, each plane's cluster carries its own Gateway API CRDs, which move from **v1.4.1 (experimental channel)** in v1.1 to **v1.5.1 (standard channel)** in v1.2. The **control plane** and **observability plane** need an extra manual step during that CRD upgrade, because their Helm releases include a `TLSRoute` at the `v1alpha2` version that the standard channel no longer serves. See [Gateway API CRD migration](#gateway-api-crd-migration-multi-cluster). The data and workflow planes render no `TLSRoute` and are unaffected, and single-cluster installations can skip it entirely.
:::
...
## Gateway API CRD migration (multi-cluster)

:::note When this applies
Only **multi-cluster** deployments — where each plane runs in its own cluster and you manage the Gateway API CRDs on each cluster yourself — need this section. Single-cluster installations can skip it.

Among the planes, only the **control plane** and the **observability plane** are affected: their Helm releases include a `TLSRoute` (the plane's gateway route) rendered at `gateway.networking.k8s.io/v1alpha2`. The **data** and **workflow** planes render no `TLSRoute` and upgrade without these steps.
:::

v1.1 ships Gateway API **v1.4.1 (experimental channel)**; v1.2 ships Gateway API **v1.5.1 (standard channel)**. In the standard channel, `TLSRoute` is served at `v1` only — `v1alpha2` still exists in the CRD but is no longer served.

### What breaks

`helm upgrade` decodes the currently-deployed release manifest to compute its diff, mapping every object in it against the cluster's live API. As soon as you apply the v1.5.1 standard CRDs, `TLSRoute/v1alpha2` stops being served, so decoding the v1.1 release manifest — which pins the gateway route to `v1alpha2` — fails:

```
Error: UPGRADE FAILED: unable to build kubernetes objects from current release manifest:
resource mapping not found for name: "cluster-gateway-tls" ...
no matches for kind "TLSRoute" in version "gateway.networking.k8s.io/v1alpha2"
ensure CRDs are installed first
```

No Helm flag skips this decode. The fix is to keep `v1alpha2` served across the upgrade — long enough for Helm to read the old manifest — then let the v1.2 chart re-render the route at `v1`.

### Procedure

Run these on the affected plane's cluster (the control plane, and the observability plane if you run it), **before** that plane's `helm upgrade`. Add `--context <ctx>` to target the plane's cluster.

```bash
# 1. Upgrade the Gateway API CRDs to v1.5.1 (standard channel).
#    This is the CRD step of the standard upgrade; it stops serving TLSRoute v1alpha2.
kubectl apply --server-side --force-conflicts \
  -f https://github.com/kubernetes-sigs/gateway-api/releases/download/v1.5.1/standard-install.yaml

# 2. Re-serve v1alpha2 so `helm upgrade` can still decode the v1.1 release manifest.
#    The v1alpha2 schema is still present in the CRD; only its `served` flag was turned off.
...
# Verify both v1 and v1alpha2 report served=true before upgrading:
kubectl get crd tlsroutes.gateway.networking.k8s.io \
  -o jsonpath='{range .spec.versions[*]}{.name}{" served="}{.served}{"  "}{end}{"\n"}'
```

Then run the plane's `helm upgrade`. For the control plane this is [step 6](#step-6-resume-the-writers-and-upgrade-the-control-plane-helm-release) of the staged migration; for the observability plane it is the [standard upgrade](#upgrading-the-other-planes). The v1.2 chart renders `TLSRoute` at `v1`, so the upgraded release no longer references `v1alpha2`.

### Converge back to the standard channel (optional)

Once the upgrade succeeds, the release references only `TLSRoute/v1`. Re-apply the standard CRDs to return `v1alpha2` to `served: false` and match the shipped channel:
...
## Upgrading kgateway (recommended)

v1.1 installs kgateway **v2.2.1**; v1.2 aligns on **v2.3.1**.

:::note Optional
Upgrading kgateway is **recommended but optional**. v2.2.1 keeps serving traffic under v1.2 (it works with the v1.5.1 CRDs), and v2.3.1 works with both the v1.4.1 and v1.5.1 CRDs, so you can stay on v2.2.1 if you prefer. Unlike the plane upgrades, the kgateway version is independent of the OpenChoreo version.
:::

### Deployment topology

kgateway runs as one controller per gateway-serving plane, so how many times you run the upgrade depends on your deployment mode:

- **Single-cluster**: a single kgateway controller (in the `openchoreo-control-plane` namespace) serves the control, data, and observability planes. Upgrade it **once**.
- **Multi-cluster**: each plane cluster runs its own kgateway controller. Upgrade it on the **control plane**, **data plane**, and **observability plane** clusters. The workflow plane does not run kgateway.

| Plane               | kgateway namespace               |
| ------------------- | -------------------------------- |
...
Run this **before** the related plane's `helm upgrade`. For each kgateway install, upgrade its CRDs, then the controller. Add `-n <namespace>` from the table above, and in multi-cluster `--kube-context <ctx>` for the target cluster:
...
v1.1 sets `KGW_ENABLE_GATEWAY_API_EXPERIMENTAL_FEATURES=true` so kgateway can reconcile the then-experimental `TLSRoute`. v1.2 no longer needs it — `TLSRoute` is GA in the v1.5.1 standard channel — so the controller upgrade uses `--reset-values` to revert to the chart defaults instead of carrying the flag forward.

:::warning Order matters when dropping the flag
On a plane that serves a `TLSRoute` (the **control plane** and **observability plane**), run this kgateway upgrade **after** that cluster's Gateway API CRDs are on **v1.5.1 standard** (see [Gateway API CRD migration](#gateway-api-crd-migration-multi-cluster)). Dropping the experimental-features flag while the CRDs are still on v1.4.1 stops kgateway from reconciling the still-experimental route. If you need to upgrade kgateway first, keep the flag by replacing `--reset-values` with `--reuse-values`.
:::
...
:::note Multi-cluster
Before running the upgrade below, complete the [Gateway API CRD migration](#gateway-api-crd-migration-multi-cluster) on the control-plane cluster (apply the v1.5.1 standard CRDs and re-serve `TLSRoute v1alpha2`). Otherwise this upgrade fails with `no matches for kind "TLSRoute" in version "gateway.networking.k8s.io/v1alpha2"`.
:::

If you are upgrading kgateway, do it now — see [Upgrading kgateway](#upgrading-kgateway-recommended) (run it after the Gateway API CRD migration above, since the control plane serves a `TLSRoute`).
...
| `helm upgrade` fails: `no matches for kind "TLSRoute" in version "gateway.networking.k8s.io/v1alpha2"` | Multi-cluster: Gateway API CRDs were upgraded to v1.5.1 standard (which stops serving `TLSRoute v1alpha2`), but the current release manifest still references it | Re-serve `v1alpha2` before upgrading — see [Gateway API CRD migration](#gateway-api-crd-migration-multi-cluster) |
...
:::note Multi-cluster: observability plane
In a multi-cluster deployment, the observability plane also renders a `TLSRoute` and needs the [Gateway API CRD migration](#gateway-api-crd-migration-multi-cluster) (apply the v1.5.1 standard CRDs and re-serve `v1alpha2`) before its `helm upgrade`. The data and workflow planes do not.
:::

Optionally [upgrade kgateway](#upgrading-kgateway-recommended) on each plane before its `helm upgrade` (data plane and, in multi-cluster, the observability plane).
