...
This creates a cluster named `openchoreo`.
...
[External Secrets Operator](https://external-secrets.io/) syncs secrets from external providers into Kubernetes. OpenChoreo uses it to pull secrets from a `ClusterSecretStore` into the right namespaces.
...
```bash
kubectl logs -n thunder -l app.kubernetes.io/name=thunder --tail=50
```
...
The ca-extractor job has been removed from the control plane helm chart. You now need to manually extract the cluster-gateway CA certificate from the cert-manager Secret and populate the ConfigMap that the controller-manager and cluster-agents use to verify the gateway server.
