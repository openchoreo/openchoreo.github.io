...
OpenChoreo uses [External Secrets Operator](https://external-secrets.io/) to manage secrets. All secrets are stored in a `ClusterSecretStore` named `default` and synced into the right namespaces using `ExternalSecret` resources. This guide uses [OpenBao](https://openbao.org/) (an open-source Vault fork) as the secret backend. You can swap it for any [ESO-supported provider](https://external-secrets.io/latest/provider/) by replacing the `default` ClusterSecretStore. See [Secret Management](../../platform-engineer-guide/secret-management.mdx) for details.
...
```bash
# Get the built image URL from the publish-image step
...
# If the above returns empty, check the publish-image pod logs directly
if [ -z "$IMAGE" ]; then
...
Wait for OpenSearch to be ready before proceeding. The setup jobs need a running OpenSearch instance to configure security credentials:

```bash
kubectl rollout status statefulset -l app.kubernetes.io/name=opensearch \
  -n openchoreo-observability-plane --timeout=300s
```

:::caution[OpenSearch setup jobs]
The `opensearch-setup-logs` and `opensearch-setup-tracing` jobs configure OpenSearch security credentials on first run. If OpenSearch isn't ready when the jobs start, they fail with `401 Unauthorized`. Delete the failed jobs and reinstall the logs/traces modules to recreate them:

```bash
kubectl delete job opensearch-setup-logs opensearch-setup-tracing \
  -n openchoreo-observability-plane 2>/dev/null
```

Then re-run the logs and traces `helm upgrade --install` commands above.
:::
