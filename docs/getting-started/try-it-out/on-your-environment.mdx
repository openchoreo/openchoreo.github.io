...
:::note[Multi-cluster deployments]
In this single-cluster guide, the agent uses the shared `cluster-gateway-ca` Secret (copied from the control plane namespace) as its signing CA. In multi-cluster deployments where `clusterAgent.tls.generateCerts=true`, the agent creates its own self-signed CA instead. In that case, extract the client CA from the `cluster-agent-tls` Secret: `kubectl get secret cluster-agent-tls -n openchoreo-data-plane -o jsonpath='{.data.ca\.crt}' | base64 -d`. See [Multi-Cluster Connectivity](../../operations/multi-cluster-connectivity.mdx) for details.
:::
...
Visit the [Modules catalog](https://openchoreo.dev/modules/) to explore available observability modules. Select the modules that match your observability requirements, then follow the installation instructions for each module.

Common observability modules include:
- **Logs** modules for centralized log collection and storage
- **Metrics** modules for monitoring performance
- **Tracing** modules for distributed tracing
