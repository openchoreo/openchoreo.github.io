...
Same process as the data plane. Copy the cluster-gateway CA from the cert-manager Secret so the build plane's agent can connect to the control plane:
...
OpenSearch expects a secret with admin credentials. Furthermore, Observer API needs credentials to connect to OpenSearch. This guide uses the same credentials for both and pulls values from the `ClusterSecretStore` created earlier. You can configure access control in OpenSearch and use separate credentials if required.
...
observer:
  ...
  controlPlaneApiUrl: "http://api.placeholder.tld"
  http:
    hostnames:
      - "observer.placeholder.tld"
  cors:
    allowedOrigins:
      - "https://console.placeholder.tld"
security:
  oidc:
    issuer: "https://thunder.placeholder.tld"
gateway:
