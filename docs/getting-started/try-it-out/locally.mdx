---
title: Locally
description: Run OpenChoreo locally with k3d. Install each plane step-by-step, verify it works, and understand what every piece does.
sidebar_position: 1
---
...
{`curl -sL https://raw.githubusercontent.com/openchoreo/openchoreo/${versions.githubRef}/install/k3d/single-cluster/config.yaml | k3d cluster create --config=-`}
</CodeBlock>
...
```bash
kubectl logs -n openchoreo-control-plane -l app=thunder-bootstrap --tail=50
```
...
You can browse and modify these in the Thunder admin console at [http://thunder.openchoreo.localhost:8080/develop](http://thunder.openchoreo.localhost:8080/develop).
