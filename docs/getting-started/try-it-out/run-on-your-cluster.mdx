...
## Step 3: Configure Control Plane Domain & TLS
...
Convert the IP to nip.io format (dots to dashes):
...
### Upgrade Control Plane with TLS
...
## Step 5: Configure Data Plane Domain & TLS
...
### Upgrade Data Plane with Domain
...
{`helm upgrade --install openchoreo-data-plane ${versions.helmSource}/openchoreo-data-plane \\
    ...
    --namespace openchoreo-data-plane \\
    --reuse-values \\
    --set global.baseDomain=\${DP_DOMAIN} \\
    --set global.tls.enabled=true`}
</CodeBlock>
...
## Step 6: Install Build Plane (Optional)

:::note Single Cluster Setup
In a single-cluster setup, external-secrets CRDs are already installed by the data plane. Use `--set external-secrets.enabled=false` to avoid conflicts.
:::

### Create Issuer and Certificate for Build Plane
