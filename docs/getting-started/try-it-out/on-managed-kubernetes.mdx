...
{`helm upgrade --install openchoreo-control-plane ${versions.helmSource}/openchoreo-control-plane \\
    ...
    --set security.oidc.issuer=http://0.0.0.0:8090`}
</CodeBlock>
...
- `security.oidc.issuer`: required workaround for v0.14.x token issuer validation.
...
This guide intentionally keeps the data plane on self-signed TLS. If you need trusted certificates for data-plane app endpoints, use your own domain with DNS-01 and a custom ClusterIssuer.
:::
