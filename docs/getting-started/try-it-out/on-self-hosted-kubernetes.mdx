...
{`helm upgrade --install openchoreo-data-plane ${versions.helmSource}/openchoreo-data-plane \\
    ...
    --set gateway.tls.clusterIssuer=openchoreo-selfsigned-issuer \\
...
Configure the registry endpoint to use the port-forwarded address that kubelet can resolve:
...
Configure containerd on each KIND node so kubelet can resolve the internal registry:

```bash
# Wait for the registry service to be ready
kubectl wait --for=condition=available deployment/registry -n openchoreo-build-plane --timeout=60s

# Get registry ClusterIP
REGISTRY_IP=$(kubectl get svc registry -n openchoreo-build-plane -o jsonpath='{.spec.clusterIP}')

# Configure each node
for node in $(kind get nodes --name openchoreo); do
  # Add containerd registry config
  docker exec $node sh -c '
    mkdir -p /etc/containerd/certs.d/registry.openchoreo-build-plane.svc.cluster.local:5000
    cat > /etc/containerd/certs.d/registry.openchoreo-build-plane.svc.cluster.local:5000/hosts.toml <<EOF
server = "http://registry.openchoreo-build-plane.svc.cluster.local:5000"
[host."http://registry.openchoreo-build-plane.svc.cluster.local:5000"]
  capabilities = ["pull", "resolve"]
  skip_verify = true
EOF
  '
  # Add host entry so kubelet can resolve the registry DNS
  docker exec $node sh -c "echo '$REGISTRY_IP registry.openchoreo-build-plane.svc.cluster.local' >> /etc/hosts"
done
```
