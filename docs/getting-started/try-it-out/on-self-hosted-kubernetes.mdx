...
For example, to generate the machine ID for the k3d-openchoreo-server-0 node:
...
- `gateway.publicVirtualHost`: where your deployed applications become accessible. When you deploy a component later, it'll be reachable at something like `http://dev-default.openchoreoapis.localhost:19080/your-component/`.
...
{`helm upgrade --install openchoreo-build-plane ${versions.helmSource}/openchoreo-build-plane \\
    --version ${versions.helmChart} \\
    --namespace openchoreo-build-plane \\
    --set global.defaultResources.registry.host=host.k3d.internal:10082 \\
...
| Deployed Apps | `http://<env>-<component namespace>.openchoreoapis.localhost:19080/<component>/...` |
