...
#### Non-HA mode
The default configurations of the OpenChoreo Observability plane deploys OpenSearch with high availability. This requires more resources (CPU and memory). To deploy OpenSearch without high availability, install the OpenChoreo observability plane as follows. This will create the `openchoreo-observability-plane` namespace automatically.
