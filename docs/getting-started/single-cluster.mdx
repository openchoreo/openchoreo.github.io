...
**Create ObservabilityPlane Resource** (required for logs in Backstage):
...
```bash
curl -s https://raw.githubusercontent.com/openchoreo/openchoreo/${versions.githubRef}/install/add-observability-plane.sh | bash -s -- --control-plane-context k3d-openchoreo --name default --observer-url https://observer.openchoreo-observability-plane.svc.cluster.local:8080
```
...
```bash
curl -s https://raw.githubusercontent.com/openchoreo/openchoreo/${versions.githubRef}/install/add-observability-plane.sh | bash -s -- --name default --observer-url https://observer.openchoreo-observability-plane.svc.cluster.local:8080
```
...
# Verify ObservabilityPlane agent is connected (if installed)
echo "=== ObservabilityPlane Agent Status ==="
kubectl get pods -n openchoreo-observability-plane -l app=cluster-agent

echo "=== ObservabilityPlane Agent Connection Logs ==="
kubectl logs -n openchoreo-observability-plane -l app=cluster-agent --tail=5 | grep "connected to control plane"
