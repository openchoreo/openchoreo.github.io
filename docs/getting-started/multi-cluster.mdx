...
```bash
# Download the values file
curl -o values-op.yaml https://raw.githubusercontent.com/openchoreo/openchoreo/${versions.githubRef}/install/k3d/multi-cluster/values-op.yaml

# Edit values-op.yaml and replace clusterAgent.tls.serverCAValue with contents of ./agent-cas/server-ca.crt
# Then install using the local values file
helm install openchoreo-observability-plane oci://ghcr.io/openchoreo/helm-charts/openchoreo-observability-plane \
  --version ${versions.helmChart} \
  --kube-context k3d-openchoreo-op \
  --namespace openchoreo-observability-plane \
  --values values-op.yaml
```
...
- `opensearch-master-*`,`opensearch-data-*`  (Running) - Log storage backend
...
kubectl rollout restart daemonset/fluent-bit -n openchoreo-build-plane --context k3d-openchoreo-bp

# Restart FluentBit pods in Data Plane
kubectl rollout restart daemonset/fluent-bit -n openchoreo-data-plane --context k3d-openchoreo-dp

# Verify FluentBit pods are running
kubectl get pods -n openchoreo-build-plane --context k3d-openchoreo-bp | grep fluent
kubectl get pods -n openchoreo-data-plane --context k3d-openchoreo-dp | grep fluent
```
...
#### Configure ObservabilityPlane

Register the observability plane with the control plane using cluster agent mode:

**Step 1: Expose Observer Service with NodePort**

For multi-cluster setup, expose the observer service with a NodePort for cross-cluster communication:
...
kubectl patch svc observer -n openchoreo-observability-plane --type='json' \
  ...
  --context k3d-openchoreo-op
```

**Step 2: Create ObservabilityPlane Resource**

<CodeBlock language="bash">
    {`curl -s https://raw.githubusercontent.com/openchoreo/openchoreo/${versions.githubRef}/install/add-observability-plane.sh | bash -s -- \\
  --control-plane-context k3d-openchoreo-cp \\
  --observabilityplane-context k3d-openchoreo-op \\
  --name default \\
  --observer-url http://openchoreo-op-control-plane:30880/observer`}
</CodeBlock>

This script:
- Extracts the cluster agent's client CA certificate from the observability plane
- Creates an ObservabilityPlane resource in the default namespace with agent-based communication enabled
- The agent provides secure WebSocket-based connectivity between the control plane and observability plane

Verify the ObservabilityPlane was created:

```bash
# Check ObservabilityPlane resource
kubectl get observabilityplane -n default --context k3d-openchoreo-cp

# Verify agent mode is enabled
kubectl get observabilityplane default -n default --context k3d-openchoreo-cp -o jsonpath='{.spec.agent.enabled}'
```

The `agent.enabled` field should show `true`, and the Ready condition should have status `True` once the agent successfully connects to the control plane.

**Configure DataPlane to use ObservabilityPlane:**

```bash
kubectl patch dataplane default -n default --type merge \
  -p '{"spec":{"observabilityPlaneRef":"default"}}' \
  --context k3d-openchoreo-cp
```

**Configure BuildPlane** (if installed) to use ObservabilityPlane:
...
# Verify ObservabilityPlane is registered and ready (if installed)
if kubectl get observabilityplane -n default &>/dev/null; then
  echo "ObservabilityPlane found:"
  kubectl get observabilityplane -n default
  kubectl describe observabilityplane -n default | grep -A 5 "Status:"
else
  echo "ObservabilityPlane not installed - skipping verification"
fi
```
