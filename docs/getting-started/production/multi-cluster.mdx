...
kubectl --context $CP_CONTEXT get dataplane,buildplane,observabilityplane -A
...
Check observability plane agent:

```bash
kubectl --context $OP_CONTEXT logs -n openchoreo-observability-plane \
  -l app.kubernetes.io/component=cluster-agent --tail=20
```
