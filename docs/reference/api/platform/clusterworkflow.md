...
cluster-scoped, they can only reference **ClusterBuildPlanes** (not namespace-scoped BuildPlanes) in their
`buildPlaneRef` field.
...
|----------------------|---------------------------------------------------------|----------|---------|----------------------------------------------------------------------------------------------------------|
| `buildPlaneRef`      | [ClusterBuildPlaneRef](#clusterbuildplaneref)            | No       | -       | Reference to the ClusterBuildPlane for this workflow's build operations                                   |
...
References the cluster-scoped build plane where workflows execute.

| Field  | Type   | Required | Default | Description                                           |
|--------|--------|----------|---------|-------------------------------------------------------|
| `kind` | string | Yes      | -       | Must be `ClusterBuildPlane`                           |
| `name` | string | Yes      | -       | Name of the ClusterBuildPlane resource                |

If not specified, the controller resolves to the `ClusterBuildPlane` named `default`.

### Schema

See [Workflow Schema](./workflow.md#schema) for the full schema documentation. ClusterWorkflows use the same schema
...
- [ClusterBuildPlane](./clusterbuildplane.md) - Cluster-scoped build plane referenced by ClusterWorkflows
