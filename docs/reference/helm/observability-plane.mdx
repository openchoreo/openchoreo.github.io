...
| `observer.secretName` | Name of an existing Secret containing Observer credentials (required key: `oauth-client-secret`) | `string` |  |
...
| `rca.llm.secretName` | Name of an existing Secret containing the LLM API key (required key: `rca-llm-api-key`) | `string` |  |
...
| `rca.oauth.secretName` | Name of an existing Secret containing the OAuth client secret (required key: `oauth-client-secret`) | `string` |  |
| `rca.observerMcpUrl` | Observer MCP endpoint URL | `string` |  |
| `rca.remedAgent` | Enable remediation agent | `boolean` | `true` |
| `rca.reportBackend` | Report storage backend type (`sqlite` or `postgresql`) | `string` | `sqlite` |
...
| `rca.sqlBackendSecretName` | Name of an existing Secret containing the SQL backend connection URI (required key: `postgresql-uri`). Required when reportBackend is postgresql. | `string` |  |
