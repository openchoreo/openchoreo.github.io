...
| `observer.oauthClientSecret` | OAuth2 client secret for the Observer when calling the control plane API | `string` | `openchoreo-observer-secret` |
...
| `rca.sqlBackendSecret.name` | Name of an existing Secret containing the SQL connection URI (required for postgresql) | `string` |  |
| `rca.sqlBackendSecret.key` | Key within the Secret that holds the URI | `string` | `uri` |
