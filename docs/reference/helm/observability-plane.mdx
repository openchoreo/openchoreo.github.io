...
| Parameter | Description | Type | Default |
| :--- | :--- | :--- | :--- |
| `observer.alertStoreBackend` | Alert entry storage backend for fired alerts | `object` | `sqlite` |
...
| `rca.extraEnvs` | Extra environment variables for the RCA agent container | `array` |  |
...
| `rca.openchoreoApiUrl` | OpenChoreo API base URL used by rca-agent | `string` | `http://api.openchoreo.localhost:8080` |
| `rca.remedAgent` | Enable remediation agent | `boolean` | `true` |
| `rca.replicas` | Number of RCA agent replicas | `integer` | `1` |
| `rca.reportBackend` | Report storage backend type | `object` | `sqlite` |
...
| `rca.secretName` | Name of an existing Secret injected via envFrom into the RCA agent container. Required keys - RCA_LLM_API_KEY, OAUTH_CLIENT_SECRET. Optional keys - SQL_BACKEND_URI (when reportBackend is postgresql). | `string` | `rca-agent-secret` |
