...
Configuration for buildpack image caching and registry.

:::note
ClusterWorkflowTemplates (docker, react, ballerina-buildpack, google-cloud-buildpacks) are no longer deployed through this Helm chart. They are installed separately via `kubectl apply`. See the [CI Overview](../../user-guide/ci/overview.md) for details.
:::
...
| `defaultResources.enabled` | Master switch for buildpack cache resources. Must be enabled for buildpackCache to take effect. | `boolean` | `false` |
...
| `defaultResources.registry.host` | Container registry host for buildpack image caching. Used by the buildpack cache pre-pull job. | `string` | `registry.invalid` |
| `defaultResources.registry.repoPath` | Repository path prepended to cached image names. | `string` |  |
| `defaultResources.registry.tlsVerify` | Enable TLS verification when pushing cached images to the registry. | `boolean` | `false` |
