...
OpenChoreo supports the following authentication methods for MCP servers. Choose the method that best fits your AI assistant and workflow:
...
Create a new OAuth2 application with authorization code grant:
...
curl -L "${THUNDER_URL}/applications" \
  ...
  -d '{
    "name": "MCP client",
    "user_attributes": ["given_name", "email", "groups"],
    "inbound_auth_config": [
      {
        "type": "oauth2",
        "config": {
          ...
          "response_types": ["code"],
          "redirect_uris": ["http://localhost:8076/callback"],
...
:::note Redirect URIs
The `redirect_uris` in the above command should be configured according to your AI assistant's callback URL. The example uses `http://localhost:8076/callback`, but you should update this to match your AI assistant's configuration. Refer to your AI assistant's MCP documentation for the correct callback URL.
:::
...
See the [Quick Start Guide](../../getting-started/quick-start-guide.mdx#access-the-backstage-ui) for more information about default credentials.
