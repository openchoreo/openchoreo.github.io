...
<details>
<summary><strong>Subject types</strong></summary>

The Portal Assistant decides which subject a request comes from using the same rules as the OpenChoreo API. The chart renders them from `openchoreoApi.config.security.subjects` into a `portal-assistant-auth-config` ConfigMap, mounted at `/etc/openchoreo/auth-config.yaml`.

To change the claim a subject type is matched on, edit those values. For example, to match service accounts on `sub` instead of `client_id`:
...
The same values drive the OpenChoreo API, so the assistant and the API cannot disagree about who a caller is. See [Authorization](../platform-engineer-guide/authorization.md#subject-types).
