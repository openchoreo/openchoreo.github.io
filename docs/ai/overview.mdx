...
When something goes wrong in production, finding the root cause is expensive. The **[RCA Agent](./rca-agent.mdx)** automates that investigation by analyzing logs, metrics, and traces from your OpenChoreo deployments and generating a report with likely root causes — powered by an LLM of your choice. Instead of engineers spending hours correlating signals across dashboards, the RCA Agent delivers actionable insights the moment an alert fires.
