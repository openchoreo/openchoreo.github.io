...
:::note
`publicHTTPPort` and `publicHTTPSPort` must match the ports your Data Plane gateway is externally reachable on. These default to 80/443 for standard LoadBalancers. If your gateway uses non-standard ports (e.g., NodePort, port remapping, or shared-IP setups), update these values to match. The Control Plane uses them to generate correct URLs for deployed workloads.
:::
