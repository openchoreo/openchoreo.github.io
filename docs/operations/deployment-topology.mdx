...
| Port | Purpose |
|------|---------|
| 443 | Observability API, RCA Agent (optional), and ingestion endpoints |
| 80 | HTTP to HTTPS redirect (optional) |
...
| Observability Plane (Optional) | Observer, RCA Agent (optional), and ingestion endpoint domain |
...
| Observability Plane (Optional) | Standard | Observer, RCA Agent (optional), and ingestion endpoint |
...
| Cluster Agent | 50m | 100m | 128Mi | 256Mi |
| RCA Agent (optional) | 100m | 250m | 1024Mi | 1536Mi |
...
| Observability Plane | RCA Agent PVC (Optional -- for RCA reports storage) | 128Mi |
...
Namespace
├── Project
│   └── references → BuildPlane / ClusterBuildPlane (optional)
├── Environment
...
A common production topology: keep the Control Plane, Build Plane, and Observability Plane together in one cluster, while running Data Planes in separate clusters (potentially across regions). This reduces operational overhead for infrastructure planes while giving workloads their own isolated clusters.

```
┌──────────────────────────────────────────────┐
│              Management Cluster               │
├──────────────┬───────────────┬───────────────┤
│ Control Plane│  Build Plane  │ Observability │
│              │               │     Plane     │
└──────┬───────┴───────────────┴───────────────┘
       │
       ├────────────────┬────────────────┐
...
- The `gateway` field embeds a full gateway configuration (virtual hosts, ports) that can override the DataPlane's defaults per environment
...
For complete configuration options, see the [BuildPlane API Reference](../reference/api/platform/buildplane.md).
...
- `rcaAgentURL`: The RCA Agent service endpoint (optional)
- `agent`: Cluster agent configuration for communication

Both DataPlane and BuildPlane can reference an ObservabilityPlane or ClusterObservabilityPlane via `observabilityPlaneRef`.
...
- **ClusterBuildPlane** - A cluster-scoped BuildPlane that can be referenced by Projects. See the [ClusterBuildPlane API Reference](../reference/api/platform/clusterbuildplane.md).
- **ClusterObservabilityPlane** - A cluster-scoped ObservabilityPlane that can be referenced by DataPlanes, ClusterDataPlanes, BuildPlanes, or ClusterBuildPlanes. See the [ClusterObservabilityPlane API Reference](../reference/api/platform/clusterobservabilityplane.md).
...
|----------|----------------|--------|--------|
| Project | `buildPlaneRef` | BuildPlane / ClusterBuildPlane | Structured (`kind`, `name`) |
...
| BuildPlane | `observabilityPlaneRef` | ObservabilityPlane / ClusterObservabilityPlane | Structured (`kind`, `name`) |
...
- [BuildPlane API Reference](../reference/api/platform/buildplane.md)
- [ClusterBuildPlane API Reference](../reference/api/platform/clusterbuildplane.md)
