...
The RCA Agent stores its reports in OpenSearch and needs credentials to connect. Create a secret with your OpenSearch credentials. If you followed the [local setup guide](../getting-started/try-it-out/on-k3d-locally.mdx#step-7-setup-observability-plane-optional), this pulls from the `ClusterSecretStore` seeded during installation:
