...
To enable the RCA Agent, set `rca.enabled=true` and configure the LLM and upgrade the Observability Plane. For example, if you followed the [local setup guide](../getting-started/try-it-out/locally.mdx#step-7-setup-observability-plane-optional), run the following command:
...
Configure `rcaAgentURL` in the `ObservabilityPlane` resource in the control plane. For example, if you followed the [local setup guide](../getting-started/try-it-out/locally.mdx#step-7-setup-observability-plane-optional), run the following command:
