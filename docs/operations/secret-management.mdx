...
After OpenBao is running, configure Kubernetes auth and create the `ClusterSecretStore`. See the [getting started guide](../getting-started/try-it-out/on-k3d-locally.mdx#step-3-setup-secrets) for the full setup steps.
...
To use a different secret backend, create a `ClusterSecretStore` named `default` with your provider's configuration. All OpenChoreo planes reference this single store, so changing the backend is a single resource swap. See the [ESO provider documentation](https://external-secrets.io/latest/provider/) for provider-specific setup.
