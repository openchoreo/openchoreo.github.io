...
- **OpenChoreo installed** in your Kubernetes cluster (see [On Self-Hosted Kubernetes](../../../getting-started/try-it-out/on-self-hosted-kubernetes.mdx) or the [Deployment Topology guide](../../deployment-topology.mdx) for production setup)
...
│   ├── platform-kustomization.yaml # Syncs platform/ directory
│   └── projects-kustomization.yaml # Syncs projects/ (depends on platform)
├── platform/                      # Platform-level resources
│   ├── infra/                     # DataPlanes, Environments, Pipelines
│   ├── component-types/           # Reusable workload templates
│   └── traits/                    # Cross-cutting concerns
└── projects/                      # Application resources
    └── <your-project>/            # One directory per Project
        ├── project.yaml           # Project definition
        └── components/
            └── <your-component>/  # One directory per Component
                ├── component.yaml # Component definition
                ├── workload.yaml  # Workload specification
                ├── releases/      # ComponentRelease snapshots
                └── bindings/      # ReleaseBindings per Environment
```

:::tip
The `platform/` directory is synced first, ensuring Environments, DataPlanes, and ComponentTypes exist before any Components are created. Flux enforces this ordering through the `dependsOn` field in `projects-kustomization.yaml`.
:::
...
Apply all three Flux resources to your cluster:
...
- **GitRepository** (`openchoreo-sample-gitops`): Tells Flux to monitor your fork
- **Kustomization** (`openchoreo-sample-platform`): Syncs the `platform/` directory
- **Kustomization** (`openchoreo-sample-projects`): Syncs the `projects/` directory (depends on platform being ready first)
...
```bash
kubectl annotate gitrepository -n flux-system openchoreo-sample-gitops \
...
Within 1-2 minutes, Flux syncs the `platform/` directory. Verify the platform resources were created:
...
After the platform is ready, Flux syncs the `projects/` directory. Verify that your application resources have been created:
...
You should see the Projects, Components, ComponentReleases, and ReleaseBindings that correspond to the manifests in your `projects/` directory.

## Step 6: How a Component Gets Deployed

In OpenChoreo, deploying a Component to an Environment requires three key resources in the `projects/` directory:
...
Create a new file `projects/demo-project-gitops/components/greeter-service-gitops/bindings/greeter-service-gitops-staging.yaml`:
...
```bash
git add projects/demo-project-gitops/components/greeter-service-gitops/bindings/greeter-service-gitops-staging.yaml
...
```bash
kubectl annotate kustomization -n flux-system openchoreo-sample-projects \
  reconcile.fluxcd.io/requestedAt="$(date +%s)" --overwrite
...
Edit `projects/demo-project-gitops/components/greeter-service-gitops/bindings/greeter-service-gitops-development.yaml` and change the `releaseName`:
...
```bash
git add projects/demo-project-gitops/components/greeter-service-gitops/bindings/greeter-service-gitops-development.yaml
...
**Advanced flows:**
- [Build and Release Workflows](../../../user-guide/gitops/build-and-release-workflows.md) - Automate container builds and GitOps releases using ComponentWorkflows
- [Bulk Promote](../../../user-guide/gitops/bulk-promote.md) - Promote multiple components to an environment in a single operation
