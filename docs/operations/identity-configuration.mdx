...
{`helm upgrade --install openchoreo-control-plane oci://ghcr.io/openchoreo/charts/openchoreo-control-plane \\
...
<CodeBlock language="bash">
{`helm upgrade --install openchoreo-observability-plane oci://ghcr.io/openchoreo/charts/openchoreo-observability-plane \\
...
- For configuration steps, refer to the [Backstage Configuration](./backstage-configuration.mdx) guide
...
1. **Check Control Plane logs** for any authentication configuration errors:
   ...
   ```bash
   kubectl logs -n openchoreo-control-plane -l app=openchoreo-control-plane --tail=50
   ```

2. **Verify Data Plane gateway** JWT validation is working:

   ```bash
   kubectl logs -n openchoreo-data-plane -l app.kubernetes.io/name=kgateway --tail=50
   ```

3. **Test authentication flow** by accessing the Backstage UI and verifying you're redirected to your identity provider's login page

4. **Verify token validation** by making authenticated API requests and checking that tokens are properly validated
...
- [Configure Backstage](./backstage-configuration.mdx) OAuth client for the Backstage Portal
