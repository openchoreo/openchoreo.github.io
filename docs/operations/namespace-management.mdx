...
:::note
If you followed the getting-started guides, cluster-scoped planes (ClusterDataPlane, ClusterBuildPlane, ClusterObservabilityPlane) are already registered and visible to all namespaces. You only need the namespace-scoped variants below if you want physical isolation for a specific namespace.
:::
...
The Build Plane is required if developers need to build applications from source code. Without it, you can only deploy pre-built container images. If a ClusterBuildPlane is already registered, new namespaces can use it automatically.
...
# Create namespace-scoped BuildPlane resource
kubectl apply -f - <<EOF
apiVersion: openchoreo.dev/v1alpha1
kind: BuildPlane
metadata:
