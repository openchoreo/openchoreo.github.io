...
Enforcement is **off by default** on this release (`clusterGateway.internalMtls.enabled=false`) to preserve upgrade compatibility. Set `clusterGateway.internalMtls.enabled=true` to require a certificate signed by the internal CA on every internal-API call and close unauthenticated access to `:8444` — after which any tooling that reaches it from outside this Helm release (debugging scripts, `kubectl port-forward` workflows) needs such a certificate, or it is rejected during the TLS handshake.
:::
