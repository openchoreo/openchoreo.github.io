...
:::warning
Do not change the key name `git-token`. This key is referenced by the default ComponentWorkflows during the checkout source step.
:::
...
3. **Checkout source step** reads the token from the mounted secret, constructs an authenticated URL, and clones the repository
...
The ComponentWorkflowRun controller creates this ExternalSecret in the Build Plane before executing the workflow, and the checkout source step mounts it as a volume.
