...
The Observability Plane MCP server provides tools covering logs, traces, metrics, alerts, incidents, audit logs, platform logs and FinOps cost insights:
...
- `query_dora_metrics` — Query DORA delivery performance metrics — deployment frequency, lead time for changes, change failure rate and mean time to recovery — for a namespace within a time range, optionally scoped to a project, component or environment; returns each metric's value, its Elite/High/Medium/Low classification and its trend against the previous window of equal length, plus a time-bucketed series at daily, weekly or monthly granularity. Requires the `deliveryinsights:view` permission. See [Delivery Insights](../platform-engineer-guide/delivery-insights.mdx)
- `query_dora_deployments` — List the individual rollouts behind the DORA metrics, each with its project, component, environment, commit, outcome and lead time; takes the same scope and window as `query_dora_metrics`, with `limit` and `sort_order` for reading the most recent first. Useful for showing the work behind a number — which deployments failed, what shipped in a window, which rollout took longest. Requires the `deliveryinsights:view` permission. See [Delivery Insights](../platform-engineer-guide/delivery-insights.mdx)
...
- `query_platform_logs` — Query platform logs: everything one observability plane collects, addressed by raw Kubernetes coordinates, including OpenChoreo's own components, the third-party infrastructure deployed alongside them and user workloads; filter by cluster, Kubernetes namespace, pod, container, pod label selector (e.g. `openchoreo.dev/plane=controlplane`), log levels and a search phrase within a time range; optionally returns a breakdown of which clusters, namespaces, pods or containers produced the matches. Requires the cluster-scoped `platformlogs:view` permission. See [Platform Logs](../platform-engineer-guide/platform-logs.mdx)
