...
- Fluent Bit was not restarted after audit collection was enabled with `helm upgrade`, so it still runs without the audit rules. Restart it as in [Step 1](#step-1-collect-audit-records-in-the-logs-module), in every cluster where the module was upgraded.
