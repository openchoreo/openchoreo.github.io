...
The Backstage container's `OPENCHOREO_EVENTS_ENABLED` env var is wired directly from `eventForwarder.enabled` — there is no separate `backstage.events.enabled` chart value. See the [Helm chart reference](../reference/helm/control-plane.mdx) for full `eventForwarder.*` configuration.
...
kubectl get pods -n thunder -l app.kubernetes.io/name=thunderid
```
