...
Run the following command. `--with-build` adds the workflow plane and `--with-observability` adds the observability plane and turns on [audit logs](../../platform-engineer-guide/audit-logging.mdx). Drop either flag to skip that plane.
...
[ThunderID](https://github.com/thunder-id/thunderid) handles authentication and OAuth flows. The values file declares the organization, users, groups, and OAuth applications as resource documents under the chart's `bootstrap.scripts` key, which the setup job — a pre-install helm hook — applies on the first install.
...
The web console (Backstage) needs a backend secret for session signing and an OAuth client secret to authenticate with ThunderID. This pulls values from the `ClusterSecretStore` created earlier:
...
:::note[Internal API authentication]
On this version, mutual TLS on the Cluster Gateway's **internal API** (port `:8444`) is **enabled by default**, and the chart wires up the internal CA and client certificates automatically — so this single-cluster setup works end to end with no extra steps. If you want to reach `:8444` directly for debugging (for example via `kubectl port-forward`), you'll need a client certificate signed by the internal CA, or you can disable enforcement by appending `--set clusterGateway.internalMtls.enabled=false` to the install command above. See [Multi-Cluster Connectivity](../../platform-engineer-guide/multi-cluster-connectivity.mdx) for the full trust model.
:::
...
You can browse and modify the bootstrapped identity configuration (users, groups, OAuth applications) in the ThunderID console at [http://thunder.openchoreo.localhost:8080/console](http://thunder.openchoreo.localhost:8080/console). Log in as `admin`, with the password ThunderID generates on install:

```bash
kubectl get secret -n thunder thunder-admin-credentials \
...
### Enable Build Caching (Optional)

Source builds pull several images on every run (builder, run, lifecycle, and Dockerfile base images) and rebuild the same layers repeatedly. You can add a cluster-internal cache to the workflow plane using [Zot](https://zotregistry.dev/), a lightweight OCI registry, to cut down build times. This sets up two independent caches:

- **Mirror cache** — a pull-through cache for upstream registries (Docker Hub, GCR, GHCR, Quay).
- **Layer cache** — a registry-backed cache for Dockerfile and Cloud Native Buildpacks build layers.

This requires the workflow plane from Step 6, and [`yq`](https://github.com/mikefarah/yq) v4 on the machine running these commands.

:::warning Use mikefarah's yq, not the Python one
The patch steps below use `strenv()` and `style="literal"`, which only exist in [mikefarah/yq](https://github.com/mikefarah/yq) v4. A different tool of the same name — the Python `yq`, which `apt install yq` installs on Ubuntu — wraps jq instead, and fails these commands with `jq: error: syntax error, unexpected IDENT` pointing at `style`. The loop still prints `Patched <workflow>` when this happens, so check your version first:

```bash
yq --version   # expect: yq (https://github.com/mikefarah/yq/) version v4.x
```
...
Zot runs as an internal ClusterIP service on port `5100` in the `openchoreo-workflow-plane` namespace.
...
<CodeBlock language="bash">
  {`helm upgrade --install build-cache project-zot/zot \\
  -n openchoreo-workflow-plane --create-namespace \\
  -f https://raw.githubusercontent.com/openchoreo/openchoreo/${versions.githubRef}/install/k3d/build-cache/values-zot.yaml`}
</CodeBlock>
...
#### Patch the Build Templates

Update the in-cluster `ClusterWorkflowTemplate` resources so builds accept cache parameters and mount the mirror config:

<CodeBlock language="bash">
  {`BASE=https://raw.githubusercontent.com/openchoreo/openchoreo/${versions.githubRef}/install/k3d/build-cache/workflow-templates
...
#### Patch the CI Workflows

Patch the CI `ClusterWorkflow` resources so component authors can control caching with a structured `cache` parameter. This adds the `cache` schema, passes layer-cache settings to the build templates, and creates a per-run `registries-conf` ConfigMap when mirror caching is enabled:
...
Once patched, components control caching with a `cache` parameter:
...
| Mode       | Read cache | Write cache | Use case                                       |
| ---------- | ---------- | ----------- | ---------------------------------------------- |
| `disabled` | No         | No          | Fully uncached build                           |
| `reuse`    | Yes        | Yes         | Normal fast builds                             |
| `rebuild`  | No         | Yes         | Clean build that refreshes the cache for later |

See the <Link to={`https://github.com/openchoreo/openchoreo/blob/${versions.githubRef}/install/k3d/build-cache/README.md`}>build cache guide</Link> for a deeper look at how caching works—image mirroring, layer cache reference paths, and how builds gracefully degrade when the cache is unavailable.
...
`observer.audit.enabled=true` makes the Observer record [audit logs](../../platform-engineer-guide/audit-logging.mdx) for its state-modifying operations and for reads of the audit trail.

#### Install the OpenSearch-based logs module

export const logsModuleInstall = `helm upgrade --install observability-logs-opensearch \\
...
<CodeBlock language="bash">{logsModuleInstall}</CodeBlock>

`auditLogs.enabled=true` routes audit records to their own `audit-logs-*` indices once log collection is enabled below.

#### Install the OpenSearch-based traces module

export const tracesModuleInstall = `helm upgrade --install observability-traces-opensearch \\
  oci://ghcr.io/openchoreo/helm-charts/observability-tracing-opensearch \\
  --create-namespace \\
  --namespace openchoreo-observability-plane \\
  --version ${versions.tracingOpensearchModule} \\
  --set openSearch.enabled=false \\
  --set openSearchSetup.openSearchSecretName="opensearch-admin-credentials"`;

<CodeBlock language="bash">{tracesModuleInstall}</CodeBlock>

#### Install the Prometheus-based metrics module

export const metricsModuleInstall = `helm upgrade --install observability-metrics-prometheus \\
  oci://ghcr.io/openchoreo/helm-charts/observability-metrics-prometheus \\
  --create-namespace \\
  --namespace openchoreo-observability-plane \\
  --version ${versions.metricsPrometheusModule}`;

<CodeBlock language="bash">{metricsModuleInstall}</CodeBlock>

#### Enable logs collection in the configured logs module

export const logsCollectionEnable = `helm upgrade observability-logs-opensearch \\
  oci://ghcr.io/openchoreo/helm-charts/observability-logs-opensearch \\
  --namespace openchoreo-observability-plane \\
  --version ${versions.logsOpensearchModule} \\
  --reuse-values \\
...
export const eventsCollectorInstall = `helm upgrade --install observability-events-otel-collector \\
  oci://ghcr.io/openchoreo/helm-charts/observability-events-otel-collector \\
  --namespace openchoreo-observability-plane \\
  --version ${versions.eventsOtelCollectorModule} \\
  -f - <<'EOF'
collector:
  extraEnv:
...
<CodeBlock language="bash">{eventsCollectorInstall}</CodeBlock>
...
### Enable Audit Logs on the Control Plane

Now that the observability plane is registered, have the control plane record [audit logs](../../platform-engineer-guide/audit-logging.mdx) and point clients at the observability plane that stores them:
...
Changes you make from now on, in the console, with `occ` or through MCP, appear under **Audit Logs** in the console sidebar and in `occ auditlogs`.
...
The ThunderID bootstrap also creates one user for each default role. See [Default Credentials](on-your-environment.mdx#default-credentials) for the full list.
