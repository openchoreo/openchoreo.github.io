...
The RCA Agent currently supports the [OpenAI](https://platform.openai.com/) GPT model series (e.g., `gpt-5.4`, `gpt-5.2-pro`, `gpt-5` etc.). Support for additional model providers is coming soon.
:::
...
By default, OpenChoreo configures ThunderID as the identity provider for the RCA Agent with a pre-configured OAuth client for testing purposes. If you are using an external identity provider, follow the steps below.
...
Add the `SQL_BACKEND_URI` key to the ExternalSecret from [Step 1](#step-1-create-the-rca-agent-secret):
