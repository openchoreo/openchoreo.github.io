...
Asgardeo Thunder (Platform Identity Provider) configuration. Thunder is installed separately; these values configure the control plane's HTTPRoute and service reference.

| Parameter | Description | Type | Default |
| :--- | :--- | :--- | :--- |
| `thunder.enabled` | Enable Thunder HTTPRoute and related resources | `boolean` | `true` |
| `thunder.host` | Thunder hostname for HTTPRoute | `string` | `thunder.openchoreo.invalid` |
| `thunder.name` | Name of the Thunder Kubernetes Service to route traffic to | `string` | `thunder-service` |
| `thunder.service.port` | Thunder service port | `integer` | `8090` |
