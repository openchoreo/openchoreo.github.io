...
For production deployments, you can provide a pre-existing Kubernetes Secret containing Backstage credentials instead of letting the chart create one. This is useful when managing secrets through External Secrets Operator or other secret management tools.

The secret must contain these keys:
- `backend-secret`: Backstage session encryption key
- `client-secret`: OAuth client secret
- `postgres-password`: PostgreSQL password (when using PostgreSQL)
...
When `existingSecret` is set, the chart skips creating its own Secret resource and references the provided one for all credential lookups.
...
| `backstage.database.postgresql.password` | Database password (required unless `existingSecret` is set) | `""` |
