...
- **OpenChoreo installed** in your Kubernetes cluster (see [On Self-Hosted Kubernetes](./try-it-out/on-self-hosted-kubernetes.mdx) or the [Operations Guide](../operations/deployment-topology.mdx) for production setup)
...
```bash
curl http://default.development.openchoreoapis.localhost:19080/greeter-service/greeter/greet
```
